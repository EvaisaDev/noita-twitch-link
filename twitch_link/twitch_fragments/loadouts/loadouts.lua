...
if #loadouts > 0 then
	for idx, loadout in pairs(loadouts) do
		...
		if(string.match(CONTENT[loadout].options.name, "TYPE"))then
			name = string.lower(CONTENT[loadout].options.name:gsub("TYPE", spellcaster_types[math.random( 1, #spellcaster_types )])):gsub("^%l", string.upper):gsub("A ",""):gsub("^%l", string.upper)
		else
			name = string.lower(CONTENT[loadout].options.name.." "..spellcaster_types[math.random( 1, #spellcaster_types )]):gsub("^%l", string.upper):gsub("A ",""):gsub("^%l", string.upper)
		end
